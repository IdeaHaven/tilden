...
configData.general = {
  ...
  flatFileDirectory: process.cwd() + "/client/.tmp/",               // The directory which will be the root for the /public route
  ...
  pidFileDirectory: process.cwd() + "/serverAPI/pids/",           // the location of the directory to keep pidfiles
  simultaniousActions: 5                                          // how many pending actions can a single connection be working on 
};

console.log(configData.general.flatFileDirectory);
...
configData.logger = {
  transports: [
    ...
    function(api, winston){
      var fs = require('fs');
      try{
        fs.mkdirSync("serverAPI/log");
        console.log("created serverAPI/log directory");
      }catch(e){
        if(e.code != "EEXIST"){
      ...
      return new (winston.transports.File)({
        filename: 'serverAPI/log/' + api.pids.title + '.log',
        level: "info",
        timestamp: true
      });
    }
